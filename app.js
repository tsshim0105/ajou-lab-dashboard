...
const filter=rows=>rows.filter(r=>($('year').value==='all'||String(r.year)===$('year').value)&&searchKey(JSON.stringify(r)).includes(searchKey($('query').value.trim())));
...
function render(){nav();exportRows=[];const panel=$('panel'),p=filter(data.papers),c=filter(data.conferences);const title=menu.find(([k])=>k===tab)[1];let html=`<h1>${title}</h1>`;
 if(tab==='home'){
  ...
  html+='<div class="note">한글·영문 저자명으로 검색할 수 있습니다. 페이지가 없는 논문은 논문번호를 표시합니다. 출판일은 해당 호·권의 발행일이며, 확인된 연도·월·일까지만 표시합니다.</div>';
  ...
  const authorText=r=>r.authorList?.length?r.authorList.map(a=>paperAuthorName(a)+(a.equalContribution?'†':'')+(a.corresponding?'*':'')).join(', '):r.authorsText||'미기재';
  const rows=records.map((r,i)=>[i+1,r.year,r.title,r.journal||'미기재',r.firstAuthor||'미입력',paperCorresponding(r)||'미입력',r.volume||'—',r.issue||'—',[r.pages,r.articleNumber?'논문번호 '+r.articleNumber:''].filter(Boolean).join(' / ')||'—',r.onlineDate||'미확인',r.issueDate||'미확인']);exportRows=[headers,...rows];
  html+=toolbar(rows.length)+table(headers,rows.map((row,i)=>{const cells=row.map(esc),r=records[i];
   if(/^https:\/\//i.test(r.publicationUrl||''))cells[2]=`<a href="${esc(r.publicationUrl)}" target="_blank" rel="noopener noreferrer">${esc(r.title)} ↗</a>`;
   cells[2]+=`<div style="display:flex;align-items:flex-start;gap:10px;margin-top:8px">${admin()?`<button class="primary green" style="white-space:nowrap" data-edit-paper="${data.papers.indexOf(r)}">수정</button>`:''}<details style="margin:0"><summary>상세 정보</summary><p style="white-space:normal"><strong>전체 저자</strong><br>${esc(authorText(r))}</p><p>† 공동 제1저자 · * 교신저자</p>${r.doi?`<p>DOI: ${esc(r.doi)}</p>`:''}<p>연계 과제: ${esc(r.fund||'미기재')}<br>등록 구분: ${esc(r.status||'미기재')}</p>${r.bibliographySource&&/^https:\/\//i.test(r.bibliographySource)?`<p><a href="${esc(r.bibliographySource)}" target="_blank" rel="noopener noreferrer">출판 정보 출처 ↗</a></p>`:''}${r.issueSource&&/^https:\/\//i.test(r.issueSource)?`<p><a href="${esc(r.issueSource)}" target="_blank" rel="noopener noreferrer">해당 호 목차 ↗</a></p>`:''}${r.paperEditedFields?.length?'<p class="muted">직접 수정됨</p>':''}</details></div>`;
   cells[10]=`<span title="${esc(r.bibliographyDateBasis||'')}">${esc(row[10])}</span>`;return cells;}),['nowrap','nowrap','title','title','title','title','nowrap','nowrap','nowrap','nowrap','nowrap']);
 }else if(tab==='conferences'){
  const records=c.slice().sort((a,b)=>b.year-a.year||(b.date||'').localeCompare(a.date||''));const headers=['발표일','발표자','학회명','국내/국제','도시','장소','발표 제목','유형','비고'];const rows=records.map(r=>[r.date||'미기재',r.presenter,r.conference,conferenceScope(r)||'미기재',conferenceCity(r)||'미기재',r.venue||'미기재',r.title||'미기재',r.kind,r.note]);exportRows=[headers,...rows];html+=`<div class="toolbar"><span class="muted">${admin()?'행의 수정 버튼을 누르면 표에서 바로 편집할 수 있습니다.':''}</span>${admin()?'<button class="primary" id="addConference">발표 내역 추가</button>':''}</div>`+toolbar(rows.length)+table([...headers,...(admin()?['관리']:[])],rows.map((r,i)=>[...r.map(esc),...(admin()?[`<button class="secondary" style="white-space:nowrap" data-conf="${data.conferences.indexOf(records[i])}">수정</button>`]:[])]),['nowrap','nowrap','title','nowrap','nowrap','title','title','nowrap','']);
...
function editConferenceRow(index,tr){
 ...
 tr.innerHTML=[input('date','발표일','date','min="1900-01-01" max="2200-12-31"')+`<button type="button" class="secondary" data-calendar style="white-space:nowrap;margin-top:6px">달력 열기</button>`,input('presenter','발표자','text','required maxlength="300"'),area('conference','학회명'),conferenceScopeField(r,true),`<input data-key="city" aria-label="도시" value="${esc(conferenceCity(r))}" maxlength="500" style="width:100%;min-width:110px;padding:7px">`,input('venue','장소','text','maxlength="1000"'),area('title','발표 제목'),`<select data-key="kind" aria-label="발표 유형">${['구두','포스터','미기재'].map(k=>`<option ${r.kind===k?'selected':''}>${k}</option>`).join('')}</select>`,area('note','비고'),'<div style="display:grid;gap:8px;min-width:72px"><button class="primary" data-save>저장</button><button class="secondary" data-cancel>취소</button></div><p role="status" style="min-width:100px"></p>'].map(v=>`<td>${v}</td>`).join('');
...
function paperAuthorName(a){return a.korean?a.korean+(a.nameMatch==='romanization'?' [이름 확인 필요]':'')+' ('+a.name+')':a.name;}
function paperCorresponding(r){return typeof r.correspondingAuthor==='string'?r.correspondingAuthor:(r.authorList||[]).filter(a=>a.corresponding).map(paperAuthorName).join(', ');}
function paperLocation(r){return [r.volume?'Vol. '+r.volume:'',r.issue?'No. '+r.issue:'',r.pages?'pp. '+r.pages:'',r.articleNumber?'논문번호 '+r.articleNumber:''].filter(Boolean).join(' · ');}
...
function editPaper(index){
 const r=data.papers[index];const fields=[['title','논문 제목 *'],['year','게재 연도 *','number'],['journal','학술지 *'],['firstAuthor','제1저자'],['correspondingAuthor','교신저자'],['volume','권 (Volume)'],['issue','호 (Issue)'],['pages','페이지 (예: 419–425)'],['articleNumber','논문번호 / eLocator'],['issueDate','호·권 출판일 (YYYY-MM-DD / YYYY-MM / YYYY)'],['onlineDate','온라인 공개일 (YYYY-MM-DD / YYYY-MM / YYYY)'],['doi','DOI'],['publicationUrl','논문 링크','url'],['fund','연계 과제'],['status','등록 구분']];
 editor('논문 정보 수정',fields.map(([k,label,type])=>field(k,label,k==='correspondingAuthor'?paperCorresponding(r):r[k],type||'text',(k==='year'?'required min="1900" max="2200" step="1"':k==='title'||k==='journal'?'required maxlength="2000"':'maxlength="2000"'))).join('')+'<p class="muted">확인되지 않은 값은 비워두세요. 출판일은 확인된 연도·월·일까지만 입력할 수 있습니다.</p>',(v,next)=>{
...
function editPaperAuthor(index){const r=data.papers[index];editor('논문 제1저자 입력',`<p>${esc(r.title)}</p>`+field('firstAuthor','제1저자 이름 *',r.firstAuthor,'text','required maxlength="500"'),(v,next)=>{const author=v.firstAuthor.trim();if(!author)throw Error('제1저자를 입력해주세요.');next.papers[index]={...r,firstAuthor:author,paperEditedFields:[...new Set([...(r.paperEditedFields||[]),'firstAuthor'])]};});}
