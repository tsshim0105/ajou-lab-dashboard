...
function render(){if(!admin()&&!['home','papers','conferences','meetingInfo'].includes(tab))tab='home';$('userLabel').innerHTML=badge(studentPreview?'학생 화면 미리보기':admin()?'교수님':'연구실 구성원');nav();exportRows=[];const panel=$('panel'),p=tab==='home'?homePeriod(data.papers):filter(data.papers),c=tab==='home'?homePeriod(data.conferences):filter(data.conferences);const title=tab==='home'?'연구실 현황 (2015년~현재)':menu.find(([k])=>k===tab)[1];let html=tab==='papers'&&admin()?`<div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:20px"><h1 style="margin:0">${title}</h1><button class="primary" id="addPaper">논문추가</button></div>`:`<h1>${title}</h1>`;
 if(tab==='home'){
