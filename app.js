...
function render(){nav();exportRows=[];const panel=$('panel'),p=filter(data.papers),c=filter(data.conferences);const title=menu.find(([k])=>k===tab)[1];let html=`<h1>${title}</h1>`;
 if(tab==='home'){
  ...
  html+='<div class="note"><a href="https://nisml.ajou.ac.kr/?page_id=234" target="_blank" rel="noopener noreferrer">연구실 홈페이지</a> 기준으로 저자·저널 정보를 갱신했습니다. 한글·영문 저자명, 제목, 저널로 검색할 수 있습니다. † 공동 제1저자 · * 교신저자. 로마자에서 추정한 한글 이름은 전체 저자에서 확인 필요로 표시합니다.</div>';
  const headers=['연도','논문 제목','학술지','제1저자','전체 저자','연계 과제','등록 구분'];
  ...
  const rows=records.map(r=>[r.year,r.title,r.citation||r.journal||'미기재',r.firstAuthor||'미입력',authorText(r),r.fund||'미기재',r.status]);exportRows=[headers,...rows];
  html+=toolbar(rows.length)+table(headers,rows.map((row,i)=>{const cells=row.map(esc),r=records[i];if(/^https:\/\//i.test(r.publicationUrl||''))cells[1]=`<a href="${esc(r.publicationUrl)}" target="_blank" rel="noopener noreferrer">${esc(r.title)} ↗</a>`;if(r.authorList?.length)cells[4]=`<details style="margin:0"><summary>${r.authorList.length}명${r.authorNamesKo?.length?' · '+esc(r.authorNamesKo.join(', ')):''}</summary><p style="min-width:240px;max-width:400px;white-space:normal">${esc(authorText(r))}</p></details>`;return cells;}),['nowrap','title','title','title','title','','nowrap']);
