...
let notifications=[];
let data=LabData.empty(),user={role:'student'},version=0,students=[],labLeadAuthor='',labMemberAuthors=[],tab='home',meetingGroup='all',fundTab='internal',term='2026-2 인건비(안)',busy=false,exportRows=[];
...
function render(){nav();exportRows=[];const panel=$('panel'),p=tab==='home'?homePeriod(data.papers):filter(data.papers),c=tab==='home'?homePeriod(data.conferences):filter(data.conferences);const title=tab==='home'?'연구실 현황 (2015년~현재)':menu.find(([k])=>k===tab)[1];let html=tab==='papers'&&admin()?`<div style="display:flex;align-items:center;gap:14px;flex-wrap:wrap;margin-bottom:20px"><h1 style="margin:0">${title}</h1><button class="primary" id="addPaper">논문추가</button></div>`:`<h1>${title}</h1>`;
 if(tab==='home'){
...
(async()=>{try{if(offline){data=LAB_PREVIEW;user={email:'',role:'admin'};$('previewNotice').classList.remove('hidden');}else{const res=await fetch('/api/data',{cache:'no-store'}),result=await res.json();if(!res.ok)throw Error(result.error);({data,user,version,students}=result);notifications=result.notifications||[];labLeadAuthor=result.labLeadAuthor||'';labMemberAuthors=result.labMemberAuthors||[];}renderNotifications();years();const current=String(new Date().getFullYear());if([...$('year').options].some(o=>o.value===current))$('year').value=current;$('userLabel').innerHTML=badge(admin()?'교수님':'연구실 구성원');render();}catch(e){$('userLabel').textContent='연결 필요';$('panel').innerHTML=`<div class="note warning">${esc(e.message)}</div>`;}})();
