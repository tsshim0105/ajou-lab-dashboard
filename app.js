...
function render(){nav();exportRows=[];const panel=$('panel'),p=filter(data.papers),c=filter(data.conferences);const title=menu.find(([k])=>k===tab)[1];let html=`<h1>${title}</h1>`;
 if(tab==='home'){
  ...
  html+='<div class="note"><a href="https://nisml.ajou.ac.kr/?page_id=234" target="_blank" rel="noopener noreferrer">연구실 홈페이지</a> 와 출판사 등록정보를 기준으로 갱신했습니다. 한글·영문 저자명, 제목, 저널로 검색할 수 있습니다. † 공동 제1저자 · * 교신저자. 로마자에서 추정한 한글 이름은 전체 저자에서 확인 필요로 표시합니다.</div>';
  const headers=['연도','논문 제목','학술지 · 권·호 · 페이지','출판일','제1저자','전체 저자','연계 과제','등록 구분'];
  const records=p.slice().sort((a,b)=>b.year-a.year||(b.websiteNumber||0)-(a.websiteNumber||0));
  const authorText=r=>r.authorList?.length?r.authorList.map(a=>(a.korean?a.korean+(a.nameMatch==='romanization'?' [이름 확인 필요]':'')+' ('+a.name+')':a.name)+(a.equalContribution?'†':'')+(a.corresponding?'*':'')).join(', '):r.authorsText||'미기재';
  const dates=r=>`호·권 출판일: ${r.issueDate||'미확인'} / 온라인: ${r.onlineDate||'미확인'}`;
  const rows=records.map(r=>[r.year,r.title,paperCitation(r),dates(r),r.firstAuthor||'미입력',authorText(r),r.fund||'미기재',r.status]);exportRows=[headers,...rows];
  html+=toolbar(rows.length)+table([...headers,...(admin()?['수정']:[])],rows.map((row,i)=>{const cells=row.map(esc),r=records[i];if(/^https:\/\//i.test(r.publicationUrl||''))cells[1]=`<a href="${esc(r.publicationUrl)}" target="_blank" rel="noopener noreferrer">${esc(r.title)} ↗</a>`;
   cells[2]=`<strong>${esc(r.journal||'학술지 미기재')}</strong><div>${esc(paperLocation(r)||r.citation||'권·호·페이지 미확인')}</div>${r.doi?`<small>DOI: ${esc(r.doi)}</small>`:''}`;
   cells[3]=`<div class="nowrap">호·권 출판일 <strong title="${esc(r.bibliographyDateBasis||'')}">${esc(r.issueDate||'미확인')}</strong></div><div class="muted nowrap">온라인 ${esc(r.onlineDate||'미확인')}</div>${r.bibliographySource&&/^https:\/\//i.test(r.bibliographySource)?`<a href="${esc(r.bibliographySource)}" target="_blank" rel="noopener noreferrer">출판 정보 출처 ↗</a>`:''}${r.issueSource&&/^https:\/\//i.test(r.issueSource)?`<div><a href="${esc(r.issueSource)}" target="_blank" rel="noopener noreferrer">해당 호 목차 ↗</a></div>`:''}${r.paperEditedFields?.length?'<div class="muted">직접 수정됨</div>':''}`;
   if(r.authorList?.length)cells[5]=`<details style="margin:0"><summary>${r.authorList.length}명${r.authorNamesKo?.length?' · '+esc(r.authorNamesKo.join(', ')):''}</summary><p style="min-width:240px;max-width:400px;white-space:normal">${esc(authorText(r))}</p></details>`;
   if(admin())cells.push(`<button class="primary green" data-edit-paper="${data.papers.indexOf(r)}">수정</button>`);return cells;}),['nowrap','title','title','nowrap','title','title','','nowrap','nowrap']);
...
function editPaper(index){
 const r=data.papers[index];const fields=[['title','논문 제목 *'],['year','게재 연도 *','number'],['journal','학술지 *'],['firstAuthor','제1저자'],['volume','권 (Volume)'],['issue','호 (Issue)'],['pages','페이지 (예: 419–425)'],['articleNumber','논문번호 / eLocator'],['issueDate','호·권 출판일 (YYYY-MM-DD / YYYY-MM / YYYY)'],['onlineDate','온라인 공개일 (YYYY-MM-DD / YYYY-MM / YYYY)'],['doi','DOI'],['publicationUrl','논문 링크','url'],['fund','연계 과제'],['status','등록 구분']];
 editor('논문 정보 수정',fields.map(([k,label,type])=>field(k,label,r[k],type||'text',(k==='year'?'required min="1900" max="2200" step="1"':k==='title'||k==='journal'?'required maxlength="2000"':'maxlength="2000"'))).join('')+'<p class="muted">확인되지 않은 값은 비워두세요. 출판일은 확인된 연도·월·일까지만 입력할 수 있습니다.</p>',(v,next)=>{
