...
function render(){if(!admin()&&!['home','papers','conferences','meetingInfo'].includes(tab))tab='home';$('userLabel').innerHTML=badge(studentPreview?'학생 화면 미리보기':admin()?'교수님':'연구실 구성원');nav();exportRows=[];const panel=$('panel'),p=tab==='home'?homePeriod(data.papers):filter(data.papers),c=tab==='home'?homePeriod(data.conferences):filter(data.conferences);const title=tab==='home'?'연구실 현황 (2015년~현재)':menu.find(([k])=>k===tab)[1];let html=['papers','conferences'].includes(tab)?`<div class="research-title"><h1>${title}</h1><div class="research-title-actions">${admin()?`<button class="primary" id="${tab==='papers'?'addPaper':'addConference'}">${tab==='papers'?'논문추가':'발표 내역 추가'}</button>`:''}<button class="secondary" id="csv">CSV 저장</button></div></div>`:`<h1>${title}</h1>`;
 if(tab==='home'){
  ...
  html+=toolbar(rows.length)+table([...headers,...(admin()?['수정']:[])],rows.map((row,i)=>{const cells=row.map(esc),r=records[i];
   ...
   if(admin())cells.push(`<button class="primary green" data-edit-paper="${data.papers.indexOf(r)}">수정</button>`);return cells;}),['nowrap','nowrap','title','title','title','title','nowrap','nowrap','nowrap','nowrap','nowrap','nowrap','nowrap','nowrap'],records.map(r=>labPaperRole(r).highlight?'lab-paper':''));
 }else if(tab==='conferences'){
  const records=c.slice().sort((a,b)=>b.year-a.year||(b.date||'').localeCompare(a.date||''));const headers=['발표일','발표자','학회명','국내/국제','지역','장소','발표 제목','유형','비고'];const rows=records.map(r=>[r.date||'미기재',r.presenter,r.conference,conferenceScope(r)||'미기재',conferenceCity(r)||'미기재',r.venue||'미기재',r.title||'미기재',r.kind,r.note]);exportRows=[headers,...rows];html+=(admin()?'<p class="muted">행의 수정 버튼을 누르면 표에서 바로 편집할 수 있습니다.</p>':'')+toolbar(rows.length)+table([...headers,...(admin()?['관리']:[])],rows.map((r,i)=>[...r.map(esc),...(admin()?[`<button class="secondary" style="white-space:nowrap" data-conf="${data.conferences.indexOf(records[i])}">수정</button>`]:[])]),['nowrap','nowrap','title','nowrap','nowrap','title','title','nowrap','']);
