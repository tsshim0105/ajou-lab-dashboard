...
globalThis.LabData=(()=>{
 ...
 const used=new Set();out.papers=data.papers.map(r=>{const old=base.papers.find(p=>(p.metadataSource||p.paperEditedFields?.length)&&[p.title,...(p.titleAliases||[])].some(t=>key(t)===key(r.title)));if(!old)return r;used.add(old.id);const merged={...r,id:old.id};for(const k of ['title','year','journal','citation','authorList','authorNamesKo','authorsText','authors','firstAuthor','publicationUrl','websiteNumber','metadataSource','metadataChecked','titleAliases','status','volume','issue','pages','articleNumber','issueDate','onlineDate','doi','bibliographySource','bibliographyChecked','bibliographyDateBasis','issueSource','paperEditedFields','paperEditedAt'])if(old[k]!==undefined)merged[k]=old[k];for(const k of old.paperEditedFields||[])if(old[k]!==undefined)merged[k]=old[k];return merged;});out.papers.push(...base.papers.filter(p=>(p.metadataSource||p.paperEditedFields?.length)&&!used.has(p.id)));
